...
def find_common_ancestor(root, persons):
    ...
    # Si el ancestro común es la misma persona, subimos un nivel
    if common_ancestor.name in persons:
        parent = find_lca(root, common_ancestor.name, root.name)
...
root.right.left.left.left.left.right.right= TreeNode("KENIA")
# (Completa la construcción del árbol con tus nodos)
